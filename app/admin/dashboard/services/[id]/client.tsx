...
export default function AdminDashboardServiceDetailClient({ id }: { id: string }) {
        ...
        <div className="p-6">
          <Card>
            <CardContent className="p-12 text-center">
              ...
              <Link href="/admin/dashboard/services" className="inline-flex items-center mt-4 text-orange-600 hover:text-orange-700">
      ...
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <Link href="/admin/dashboard/services" className="p-2 hover:bg-gray-100 rounded-lg transition-colors">
                      ...
                      <div className="space-y-2">
                        {items.map((item, idx) => (
                          <div key={idx} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                            ...
                            <span className="text-sm font-bold text-blue-600 bg-blue-100 px-3 py-1 rounded-full">{item.value}</span>
                          </div>
                        ))}
                      </div>
                    </CardContent>
                ...
                <div className="space-y-6">
                  <div className="bg-gradient-to-br from-gray-50 to-gray-100 p-4 rounded-xl border-2 border-gray-200">
                    ...
                    <div className="flex items-center gap-3">
                      {request.status === '요청됨' && (
                        <>
                          <div className="bg-blue-500 p-2.5 rounded-lg"><Clock className="h-5 w-5 text-white" /></div>
                          <div><p className="text-xl font-bold text-blue-700">요청됨</p></div>
                        </>
                      )}
                      {request.status === '진행중' && (
                        <>
                          <div className="bg-amber-500 p-2.5 rounded-lg"><Wrench className="h-5 w-5 text-white" /></div>
                          <div><p className="text-xl font-bold text-amber-700">진행중</p></div>
                        </>
                      )}
                      {request.status === '완료' && (
                        <>
                          <div className="bg-emerald-500 p-2.5 rounded-lg"><CheckCircle className="h-5 w-5 text-white" /></div>
                          <div><p className="text-xl font-bold text-emerald-700">완료</p></div>
                        </>
                      )}
                      {request.status === '취소' && (
                        <>
                          <div className="bg-slate-500 p-2.5 rounded-lg"><XCircle className="h-5 w-5 text-white" /></div>
                          <div><p className="text-xl font-bold text-slate-700">취소</p></div>
                        </>
                      )}
                    </div>
                  </div>
                  ...
                  {request.status !== '완료' && request.status !== '취소' && (
                    <div>
                      ...
                      <div className="space-y-2">
                        {request.status === '요청됨' && (
                          <>
                            <button onClick={() => updateStatus('진행중')} className="w-full px-4 py-3 bg-gradient-to-r from-amber-500 to-amber-600 text-white rounded-lg font-semibold flex items-center justify-center gap-2">
                              <Wrench className="h-4 w-4" />작업 시작하기
                            </button>
                            <button onClick={() => updateStatus('취소')} className="w-full px-4 py-3 bg-white border-2 border-red-300 text-red-700 rounded-lg font-medium flex items-center justify-center gap-2">
                              <XCircle className="h-4 w-4" />요청 취소
                            </button>
                          </>
                        )}
                        {request.status === '진행중' && (
                          <>
                            <button onClick={() => updateStatus('완료')} className="w-full px-4 py-3 bg-gradient-to-r from-emerald-500 to-emerald-600 text-white rounded-lg font-semibold flex items-center justify-center gap-2">
                              <CheckCircle className="h-4 w-4" />작업 완료 처리
                            </button>
                            <button onClick={() => updateStatus('취소')} className="w-full px-4 py-3 bg-white border-2 border-red-300 text-red-700 rounded-lg font-medium flex items-center justify-center gap-2">
                              <XCircle className="h-4 w-4" />요청 취소
                            </button>
